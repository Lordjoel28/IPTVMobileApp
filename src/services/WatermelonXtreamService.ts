...
class WatermelonXtreamService {
  ...
  private async batchImportToWatermelon({
    ...
    return await database.write(async () => {
      ...
      const playlist = await database.get<Playlist>('playlists').create((playlist) => {
        ...
        if (accountInfo?.user_info?.exp_date) {
          ...
          if (!isNaN(expTimestamp)) {
            playlist.expirationDate = new Date(expTimestamp * 1000).toISOString();
          }
        }
      });
      ...
      for (let i = 0; i < categoryBatches.length; i++) {
        ...
        const categoryRecords = batch.map(cat => 
          database.get<Category>('categories').prepareCreate((category) => {
            ...
            category.channelsCount = channels.filter(ch => ch.category_id === cat.category_id).length;
          })
        );
      ...
      for (let i = 0; i < channelBatches.length; i++) {
        ...
        onProgress?.(progress, `📺 Batch ${i + 1}/${channelBatches.length} - ${batch.length} chaînes...`);
        ...
        const channelRecords = batch.map(channel => 
          database.get<Channel>('channels').prepareCreate((ch) => {
            ...
            ch.streamUrl = this.buildXtreamStreamUrl(credentials, channel.stream_id);
            
            // NORMALISATION LOGOS lors de l'import
            console.log('🔍 Logo original pour', channel.name + ':', channel.stream_icon);
            console.log('🔍 URL serveur:', credentials.url);
            
            ch.logoUrl = this.normalizeLogoUrl(channel.stream_icon, credentials.url);
            ch.streamIcon = this.normalizeLogoUrl(channel.stream_icon, credentials.url);
      ...
      console.log(`✅ Import WatermelonDB terminé: ${channels.length} chaînes, ${categories.length} catégories`);
      return playlist.id;
    });
  }
   ...
   */
  async getPlaylistWithChannels(playlistId: string, limit: number = 25000, offset: number = 0) {
    try {
   ...
   */
  async searchChannels(playlistId: string, query: string, limit: number = 25000) {
    try {
   ...
   */
  async getChannelsByCategory(playlistId: string, categoryId: string, limit: number = 25000, offset: number = 0) {
    try {
      return await database
        .get<Channel>('channels')
        .query(
  ...
  private buildXtreamStreamUrl(credentials: XtreamCredentials, streamId: string): string {
