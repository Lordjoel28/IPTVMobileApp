  ...
  async *processChannelsStream(
    ...
    try {
      while (true) {
        ...
        const { channels, remainingBuffer } = this.extractChannelsFromBuffer(buffer);
...
class ChunkedAPILoader {
  ...
  async loadChannelsByCategories(
    ...
    try {
      ...
      const allChannels = await this.loadAllChannelsStreaming(credentials, (count, memStats) => {
  ...
  private async loadCategories(credentials: XtreamCredentials): Promise<XtreamCategory[]> {
  ...
  private async loadAllChannelsStreaming(
    ...
    for await (const channelBatch of this.streamProcessor.processChannelsStream(response, onProgress)) {
...
class StreamingXtreamService {
  ...
  async importXtreamPlaylistOptimized(
    ...
    try {
      console.log('🚀 Starting Phase 3 optimized import with Smart Cache L1/L2/L3...');
      ...
      const cachedChannels = await CacheIntegrationService.getCached<XtreamChannel[]>(
        `xtream_channels_${credentials.url}_${credentials.username}_all`,
        'XTREAM_API'
      );

      if (cachedChannels && cachedChannels.length > 0) {
        console.log(`🧠 Found ${cachedChannels.length} cached channels, using cache-first approach`);
        onProgress?.(25, `🧠 Found ${cachedChannels.length} cached channels - fast track enabled`);
        
        // Fast track with cached data
        const playlistId = await this.importFromCache(cachedChannels, playlistName, credentials, onProgress);
        return playlistId;
      }

      onProgress?.(10, '🔍 No cache found - performing full import with smart caching...');
      ...
      console.log(`📊 Loaded ${channels.length} channels, caching + starting background processing...`);
      onProgress?.(35, `📺 ${channels.length} channels loaded - caching for future use...`);

      // Cache the loaded channels for future fast access
      await CacheIntegrationService.cacheChannels(credentials, channels);
      
      onProgress?.(40, `🧠 Channels cached - starting background processing...`);
      ...
      // Cleanup
      onProgress?.(100, '✅ Advanced import completed with background processing!');
  ...
  private async importFromCache(
    ...
    console.log(`⚡ Fast track import for ${cachedChannels.length} cached channels`);
    
    onProgress?.(30, '🧠 Using cached data - warming up search index...');
    
    // Warm up cache with most important data
    await CacheIntegrationService.warmUpCache('cached_playlist', cachedChannels.slice(0, 1000));
    ...
    onProgress?.(100, `✅ Fast import completed! ${cachedChannels.length} channels from cache`);
  ...
  private async createPlaylistFromCache({
    ...
    return await database.write(async () => {
      ...
      // Create playlist
      const playlist = await database.get<Playlist>('playlists').create(record => {
      ...
      for (let i = 0; i < categories.length; i += batchSize) {
        ...
        onProgress?.(progress, `Created ${Math.min(i + batchSize, categories.length)}/${categories.length} categories`);
      }
      ...
      for (let i = 0; i < channels.length; i += batchSize) {
        ...
        onProgress?.(progress, `Created ${Math.min(i + batchSize, channels.length)}/${channels.length} channels`);
      }
  ...
  private async backgroundProcessChannels(
    ...
    console.log(`🔄 Starting background processing of ${channels.length} channels...`);
  ...
  private async importToDatabaseWithWorkers({
    ...
    return await database.write(async () => {
      ...
      // 1. Create playlist
      const playlist = await database.get<Playlist>('playlists').create((playlist) => {
      ...
      await this.createCategoriesWithProgressiveUI(playlist.id, categories, (progress) => {
        onProgress?.(20 + Math.round(progress * 0.2), 'Creating categories...');
      });
      
      onProgress?.(40, `📺 Importing ${normalizedChannels.length} channels with progressive UI...`);
      ...
      await this.createChannelsWithProgressiveUI(
        ...
        normalizedChannels, 
        (progress, count) => {
          onProgress?.(40 + Math.round(progress * 0.5), `Imported ${count}/${normalizedChannels.length} channels`);
      ...
      console.log(`✅ Phase 2 import complete: ${normalizedChannels.length} channels with background processing`);
      return playlist.id;
    });
  }
  ...
  private async importToDatabase({
    ...
    return await database.write(async () => {
      ...
      await this.createChannelsInMicroBatches(playlist.id, credentials, channels, onProgress);
      ...
      console.log(`✅ Optimized import complete: ${channels.length} channels imported`);
      return playlist.id;
    });
  }
  ...
  private async createChannelsInMicroBatches(
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      onProgress?.(progress, `📺 Batch ${i + 1}/${batches.length} - ${batch.length} channels`);
      ...
      const channelRecords = batch.map(channel => 
        database.get<Channel>('channels').prepareCreate((ch) => {
          ...
          ch.name = channel.name || 'Unknown Channel';
          ch.streamUrl = this.buildStreamUrl(credentials, channel.stream_id);
          ch.logoUrl = this.normalizeLogoUrl(channel.stream_icon, credentials.url);
          ch.streamIcon = ch.logoUrl;
  ...
  private buildStreamUrl(credentials: XtreamCredentials, streamId: string): string {
  ...
  private normalizeLogoUrl(logoUrl: string, serverUrl: string): string {
    ...
    if (trimmedLogoUrl.startsWith('http://') || trimmedLogoUrl.startsWith('https://')) {
