...
export class SearchManager {
  ...
  async search(
    ...
    try {
      ...
      // Vérifier cache
      const cacheKey = `search_${btoa(
        normalizedQuery + JSON.stringify(options),
      )}`;
      if (!options.fuzzySearch) {
        // Cache seulement pour recherche exacte
        const cached = await this.storage.get(cacheKey);
        if (cached && this.isCacheValid(cached, 0.5)) {
          // 30 min cache
  ...
  private updateSearchTime(time: number): void {
    ...
    this.stats.averageSearchTime =
      this.stats.averageSearchTime * (1 - alpha) + time * alpha;
  }
