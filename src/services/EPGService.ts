...
class EPGService {
  public async fetchAndProcessEPG(url: string, playlistId: string): Promise<boolean> {
    console.log(`[EPGService] Starting EPG processing for playlist ${playlistId} from ${url}`);

    const tempPath = `${RNFS.CachesDirectoryPath}/epg_${playlistId}.xml`;

    try {
      // Step 1: Download the EPG file
      console.log(`[EPGService] Downloading EPG file from ${url} to ${tempPath}`);
      ...
      if (downloadResult.statusCode !== 200) {
        throw new Error(`Failed to download EPG file. Status code: ${downloadResult.statusCode}`);
      }
      ...
      const programs = Array.isArray(epgData.tv.programme) ? epgData.tv.programme : [epgData.tv.programme];
      ...
      const oldPrograms = await programsCollection.query(Q.where('playlist_id', playlistId)).fetch();
      ...
      for (const prog of programs) {
        const startTime = new Date(prog['@_start'].replace(/ (\d{4})/, '$1')).getTime();
        const stopTime = new Date(prog['@_stop'].replace(/ (\d{4})/, '$1')).getTime();
        ...
        if (recordsToCreate.length >= BATCH_SIZE) {
          ...
          console.log(`[EPGService] Inserted batch of ${recordsToCreate.length} programs.`);
          recordsToCreate = [];
        }
      }
      ...
      if (recordsToCreate.length > 0) {
        ...
        console.log(`[EPGService] Inserted final batch of ${recordsToCreate.length} programs.`);
      }
      ...
      try {
        ...
        console.error('[EPGService] Error cleaning up temporary file:', cleanupError);
      }
    }
  }
  ...
  public async getProgramsForChannel(channelId: string): Promise<Program[]> {
    ...
    const now = Date.now();
    try {
        const programs = await programsCollection.query(
