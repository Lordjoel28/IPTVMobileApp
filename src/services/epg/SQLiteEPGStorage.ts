...
import { FullEPGData, EPGChannel as XtreamEPGChannel, EPGProgramme as XtreamEPGProgramme } from '../XtreamEPGService';
...
export class SQLiteEPGStorage {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      console.log('🔄 [SQLiteEPG] Initialisation WatermelonDB SQLite avec persistance...');

      // Attendre que la base soit prête
      await new Promise(resolve => setTimeout(resolve, 100));

      // Vérifier la persistance existante
      const channelsCount = await this.database.get<EPGChannel>('epg_channels').query().fetchCount();
      const programmesCount = await this.database.get<EPGProgramme>('epg_programmes').query().fetchCount();

      this.isInitialized = true;

      if (channelsCount > 0 || programmesCount > 0) {
        console.log(`🎉 [SQLiteEPG] PERSISTANCE RÉUSSIE! WatermelonDB récupéré - ${channelsCount} chaînes, ${programmesCount} programmes en base`);

        // Vérifier si ce sont des données de test
        const channels = await this.database.get<EPGChannel>('epg_channels').query().fetch();
        const isTestData = channels.some(ch => ch.channelId.startsWith('test_channel_'));

        if (isTestData) {
          console.log('🧪 [SQLiteEPG] Données de test détectées - persistance fonctionnelle!');
        }
      } else {
        console.log(`📱 [SQLiteEPG] WatermelonDB initialisé (base vide) - ${channelsCount} chaînes, ${programmesCount} programmes`);
      ...
      console.error('❌ [SQLiteEPG] Erreur initialisation WatermelonDB:', error);
      this.isInitialized = true;
    }
  }
  ...
  private async diagnosticDatabase(): Promise<void> {
    try {
      ...
      try {
        const channelsCollection = this.database.get<EPGChannel>('epg_channels');
        const programmesCollection = this.database.get<EPGProgramme>('epg_programmes');
        ...
        try {
          ...
          console.log('📊 [SQLiteEPG] Collections map keys:', Object.keys(collectionsMap));
          ...
          console.error('❌ [SQLiteEPG] Erreur collections map:', mapError.message);
        }
      }
  ...
  async saveFullEPG(epgData: FullEPGData): Promise<void> {
    try {
      ...
      console.log('🎯 [SQLiteEPG] Fenêtre temporelle:', new Date(windowStart).toLocaleString(), '→', new Date(windowEnd).toLocaleString());
      ...
      console.log(`📊 [SQLiteEPG] Programmes filtrés: ${relevantPrograms.length}/${epgData.programmes.length} (${Math.round(relevantPrograms.length/epgData.programmes.length*100)}%)`);
      ...
      await this.database.write(async () => {
        ...
        const oldChannels = await this.database.get<EPGChannel>('epg_channels').query().fetch();
        const oldProgrammes = await this.database.get<EPGProgramme>('epg_programmes').query().fetch();

        console.log(`🗑️ [SQLiteEPG] Suppression: ${oldChannels.length} anciennes chaînes, ${oldProgrammes.length} anciens programmes`);
        ...
        if (oldChannels.length > 0 || oldProgrammes.length > 0) {
          ...
          await this.database.batch(
            ...
            ...oldProgrammes.map(programme => programme.prepareDestroyPermanently())
          );
          console.log('✅ [SQLiteEPG] Anciennes données supprimées');
        }

        // 2. Créer les nouvelles chaînes
        console.log(`📺 [SQLiteEPG] Préparation de ${epgData.channels.length} nouvelles chaînes...`);
        const channelActions = epgData.channels.map(channel =>
          this.database.get<EPGChannel>('epg_channels').prepareCreate(record => {
        ...
        console.log(`📋 [SQLiteEPG] Préparation de ${relevantPrograms.length} nouveaux programmes...`);
        const programmeActions = relevantPrograms.map((program, index) => {
          const startTime = parseEPGDate(program.start)!;
          const endTime = parseEPGDate(program.stop)!;

          if (index % 10000 === 0 && index > 0) {
            console.log(`  📊 [SQLiteEPG] Progression: ${index}/${relevantPrograms.length} programmes préparés`);
          }

          return this.database.get<EPGProgramme>('epg_programmes').prepareCreate(record => {
        ...
        console.log(`💾 [SQLiteEPG] Exécution batch: ${channelActions.length + programmeActions.length} opérations...`);
        ...
        for (let i = 0; i < allOperations.length; i += chunkSize) {
          ...
          console.log(`  📦 [SQLiteEPG] Chunk ${Math.floor(i/chunkSize) + 1}/${Math.ceil(totalOperations/chunkSize)} sauvegardé`);
        }

        console.log('✅ [SQLiteEPG] Transaction WatermelonDB terminée avec succès');
      });

      console.log(`✅ [SQLiteEPG] Sauvegarde WatermelonDB terminée: ${epgData.channels.length} chaînes, ${relevantPrograms.length} programmes`);
  ...
  private async verifyPersistence(): Promise<void> {
    try {
      ...
      if (channelsCount > 0 && programmesCount > 0) {
        console.log(`✅ [SQLiteEPG] Persistance vérifiée: ${channelsCount} chaînes, ${programmesCount} programmes en base`);
      } else {
        console.error(`❌ [SQLiteEPG] PROBLÈME PERSISTANCE: ${channelsCount} chaînes, ${programmesCount} programmes trouvés`);
      }
    ...
    try {
      ...
      await this.database.write(async () => {
        ...
        const testProgrammes = [
          this.database.get<EPGProgramme>('epg_programmes').prepareCreate(record => {
            record.channelId = 'test_channel_1';
      ...
      console.log(`🧪 [SQLiteEPG] Test données créées: ${channelsCount} chaînes, ${programmesCount} programmes`);

      // Forcer la synchronisation
      await this.forceDatabaseSync();

      const success = channelsCount === 2 && programmesCount === 3;
      console.log(`🧪 [SQLiteEPG] Test persistance simple: ${success ? '✅ SUCCÈS' : '❌ ÉCHEC'}`);
  ...
  async getProgrammesForChannel(
    ...
    try {
      ...
      console.log(`🔍 [SQLiteEPG] Recherche WatermelonDB programmes pour ${channelId}`);

      // Requête WatermelonDB optimisée avec index
      let query = this.database.get<EPGProgramme>('epg_programmes')
        .query(
      ...
      const result = programmes.map(prog => ({
        ...
        title: prog.title,
        description: options?.includeDescription !== false ? prog.description : undefined,
      ...
      console.log(`✅ [SQLiteEPG] ${result.length} programmes WatermelonDB trouvés pour ${channelId}`);
      return result;

    } catch (error) {
      console.error('❌ [SQLiteEPG] Erreur récupération programmes WatermelonDB:', error);
      return [];
    }
  }
   ...
   */
  async getCurrentProgramme(channelId: string): Promise<SQLiteEPGProgramme | null> {
    try {
      const now = Date.now();

      const programme = await this.database.get<EPGProgramme>('epg_programmes')
        .query(
          Q.where('channel_id', channelId),
          Q.where('start_time', Q.lte(now)),
          Q.where('end_time', Q.gte(now))
      ...
      console.error('❌ [SQLiteEPG] Erreur programme actuel WatermelonDB:', error);
      return null;
    }
  }
   ...
   */
  async searchProgrammes(searchTerm: string, limit = 50): Promise<SQLiteEPGProgramme[]> {
    try {
      const programmes = await this.database.get<EPGProgramme>('epg_programmes')
        .query(
  ...
  async getCacheStats(): Promise<{
    ...
    try {
      ...
      if (hasData) {
        const latestChannel = await this.database.get<EPGChannel>('epg_channels')
  ...
  async clearCache(): Promise<void> {
    try {
      await this.database.write(async () => {
        ...
        const channels = await this.database.get<EPGChannel>('epg_channels').query().fetch();
        const programmes = await this.database.get<EPGProgramme>('epg_programmes').query().fetch();
  ...
  async getTotalProgramsCount(): Promise<number> {
    try {
      return await this.database.get<EPGProgramme>('epg_programmes').query().fetchCount();
    } catch (error) {
  ...
  async getProgrammesChunk(offset: number, limit: number): Promise<any[]> {
    try {
      ...
      console.error('❌ [SQLiteEPG] Erreur récupération chunk programmes:', error);
      return [];
    }
  }
  ...
  private extractCategory(name: string): string {
    ...
    if (upperName.includes('SPORT')) return 'Sport';
    if (upperName.includes('NEWS') || upperName.includes('INFO')) return 'News';
    if (upperName.includes('KIDS') || upperName.includes('CARTOON')) return 'Kids';
    if (upperName.includes('MOVIE') || upperName.includes('CINEMA')) return 'Movies';
    if (upperName.includes('MUSIC')) return 'Music';
    if (upperName.includes('DOCUMENTARY')) return 'Documentary';
