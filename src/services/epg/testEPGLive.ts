...
export async function runRealisticEPGTests(): Promise<void> {
  ...
  console.log('\n📺 Test 1: Format Xtream avec EPG');
  try {
    const result1 = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.xtreamWithEPG);
  ...
  console.log('\n📺 Test 2: Format standard avec EPG');
  try {
    const result2 = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.standardWithEPG);
  ...
  console.log('\n📺 Test 3: Sans EPG');
  try {
    const result3 = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.withoutEPG);
  ...
  console.log('\n📺 Test 4: EPG invalide');
  try {
    const result4 = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.invalidEPG);
  ...
  console.log('\n📺 Test 5: Multiples EPG');
  try {
    const result5 = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.multipleEPG);
  ...
  console.log('\n📺 Test 6: EPG complexe avec paramètres');
  try {
    const result6 = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.complexEPG);
...
export async function testEPGSourceManagerIntegration(): Promise<void> {
  ...
  try {
    ...
    const parseResult = await parser.parseWithEPGDetection(REALISTIC_M3U_SAMPLES.xtreamWithEPG);
    ...
    if (parseResult.metadata.epgUrl) {
      ...
      const epgSource = await epgManager.getBestEPGSource(mockPlaylistId, parseResult.metadata);
...
export async function testEPGPerformance(): Promise<void> {
  ...
  try {
    ...
    if (duration < 5000) { // Moins de 5 secondes pour 1000 chaînes
...
export function runQuickTests(): void {
  ...
  const hasEPG1 = M3UParserWithEPG.hasIntegratedEPG(REALISTIC_M3U_SAMPLES.xtreamWithEPG);
  const hasEPG2 = M3UParserWithEPG.hasIntegratedEPG(REALISTIC_M3U_SAMPLES.withoutEPG);
  ...
  // Test extraction URL
  const epgUrl1 = M3UParserWithEPG.extractEPGUrlFromM3U(REALISTIC_M3U_SAMPLES.standardWithEPG);
  const epgUrl2 = M3UParserWithEPG.extractEPGUrlFromM3U(REALISTIC_M3U_SAMPLES.withoutEPG);
