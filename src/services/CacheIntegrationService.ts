...
import { XtreamChannel, XtreamCategory, XtreamCredentials } from './StreamingXtreamService';
...
export const CacheKeys = {
  ...
  XTREAM_CATEGORIES: (url: string, username: string) => `xtream_categories_${url}_${username}`,
  XTREAM_CHANNELS: (url: string, username: string, categoryId?: string) => 
    `xtream_channels_${url}_${username}${categoryId ? `_cat${categoryId}` : '_all'}`,
  
  // Processed data
  NORMALIZED_CHANNELS: (playlistId: string) => `normalized_channels_${playlistId}`,
...
class CacheIntegrationService {
   ...
   */
  async cacheServerInfo(credentials: XtreamCredentials, serverInfo: any): Promise<boolean> {
   ...
   */
  async cacheCategories(credentials: XtreamCredentials, categories: XtreamCategory[]): Promise<boolean> {
    const key = CacheKeys.XTREAM_CATEGORIES(credentials.url, credentials.username);
    const strategy = CacheStrategies.XTREAM_API;
  ...
  async cacheChannels(
    ...
    const key = CacheKeys.XTREAM_CHANNELS(credentials.url, credentials.username, categoryId);
   ...
   */
  async cacheSearchIndex(playlistId: string, searchIndex: Map<string, string[]>): Promise<boolean> {
   ...
   */
  async cacheUserFavorites(userId: string, favorites: string[]): Promise<boolean> {
    ...
    if (success && strategy.predictiveLoading) {
      const relatedKeys = favorites.map(streamId => CacheKeys.CHANNEL_METADATA(streamId));
   ...
   */
  async getCached<T>(cacheKey: string, strategyType: keyof typeof CacheStrategies): Promise<T | null> {
    ...
    try {
      ...
      if (data) {
        console.log(`🔗 Cache hit for ${cacheKey} in ${accessTime.toFixed(2)}ms`);
        ...
        console.log(`🔗 Cache miss for ${cacheKey} (${accessTime.toFixed(2)}ms)`);
      }
  ...
  getCacheMetrics() {
    ...
    return {
      ...stats,
      usage: usageStats,
      recommendations: this.generateOptimizationRecommendations(stats, usageStats)
    };
  }
   ...
   */
  async warmUpCache(playlistId: string, essentialChannels: XtreamChannel[]): Promise<void> {
    ...
    console.log(`🔗 Cache warm-up completed for ${highPriorityChannels.length} priority channels`);
  }
  ...
  async performIntelligentCleanup(): Promise<void> {
    ...
    if (memoryPressure > 0.8) {
      console.log('🔗 High memory pressure detected, clearing least used L1 items');
  ...
  private async cacheWithStrategy(key: string, data: any, strategy: CacheStrategy): Promise<boolean> {
    const options = {
      ...
      BackgroundWorkerService.addTask({
        ...
        onProgress: (progress, message) => {
          ProgressiveUIService.updateProgress(
            processedBatches * batchSize + Math.floor(batch.length * progress / 100),
            channels.length
          );
        },
        onComplete: async (normalizedBatch) => {
          ...
          if (processedBatches === totalBatches) {
            ...
            console.log(`🔗 Background caching completed for ${channels.length} channels`);
          }
        }
      });
    }
  ...
  private async cacheIndividualChannels(
    ...
    for (const channel of channels.slice(0, 100)) { // Limit to first 100 for performance
      const key = CacheKeys.CHANNEL_METADATA(channel.stream_id);
      await this.cacheWithStrategy(key, {
  ...
  private async cacheIndividualCategories(categories: XtreamCategory[]): Promise<void> {
    const strategy = CacheStrategies.XTREAM_API;
  ...
  private getUsageStats() {
    ...
    const totalAccessCount = patterns.reduce((sum, p) => sum + p.accessCount, 0);
    const avgAccessCount = totalAccessCount / patterns.length;
    const avgInterval = patterns.reduce((sum, p) => sum + p.avgAccessInterval, 0) / patterns.length;
  ...
  private generateOptimizationRecommendations(cacheStats: any, usageStats: any): string[] {
    const recommendations: string[] = [];
    
    // Memory pressure recommendations
    if (cacheStats.l1.memoryUsage / cacheStats.l1.maxSize > 0.8) {
      recommendations.push('Consider increasing L1 cache size or implement more aggressive eviction');
    }
    
    // Hit rate recommendations
    if (cacheStats.overall.hitRate < 0.6) {
      recommendations.push('Low hit rate detected - consider adjusting TTL or cache warming strategy');
    }
    
    // Hot keys recommendations
    if (usageStats.hotKeys.length > 5) {
      recommendations.push(`${usageStats.hotKeys.length} hot keys detected - consider prioritizing these for L1`);
    }
    
    // Access time recommendations
    if (cacheStats.overall.avgAccessTime > 50) {
      recommendations.push('Average access time is high - consider optimizing cache key structure');
    }
