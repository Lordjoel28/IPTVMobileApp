...
import UltraOptimizedM3UParser, {
  Channel,
  ParseResult,
  ParseStats,
} from '../parsers/UltraOptimizedM3UParser';
import M3UParserWithEPG, {
  ExtendedParseResult,
} from '../parsers/M3UParserWithEPG';
...
class ParserCascade {
  ...
  async parse(
    ...
    try {
      ...
      const result = await this.epgParser.parseWithEPGDetection(
        content,
        options.chunkSize,
      );
      ...
      try {
        const basicResult = await this.ultraParser.parse(
          content,
          options.chunkSize,
        );
...
export class PlaylistManager {
    ...
    try {
      ...
      if (options.enableCache !== false) {
        ...
        if (cachedResult) {
          ...
          this.stats.cacheHitRate = this.updateHitRate(
            this.stats.cacheHitRate,
            true,
          );
          return this.createImportResult(
            cachedResult.playlist,
            [],
            warnings,
            errors,
          );
      ...
      try {
        ...
        if (error instanceof NetworkError) {
          console.error(
            `❌ Network error: ${error.type} - ${error.getUserMessage()}`,
          );
      ...
      if (parseResult.metadata.epgUrl) {
        ...
        try {
          ...
          console.log(
            '✅ EPG intégré configuré automatiquement:',
            parseResult.metadata.epgUrl,
          );

          if (parseResult.metadata.epgUrl) {
            warnings.push(
              `EPG intégré détecté: ${parseResult.metadata.epgUrl}`,
            );
          }
        } catch (error) {
      ...
      try {
        ...
        console.log(
          '✅ Playlist marquée comme active dans le store:',
          playlist.id,
        );
      } catch (error) {
        console.error(
          '❌ Erreur marquage playlist active dans le store:',
          error,
        );
      }
      ...
      try {
        ...
        console.log(
          `✅ Playlist sauvegardée dans saved_m3u_playlists avec ${playlist.channels.length} chaînes`,
        );
      } catch (error) {
      ...
      console.log(
        `✅ Playlist imported: ${channels.length} channels in ${
          Date.now() - startTime
        }ms`,
      );
  ...
  async importFromXtream(
    ...
    try {
      ...
      console.log(
        `✅ Playlist Xtream importée: ${playlist.totalChannels} chaînes`,
      );
  ...
  async refreshPlaylist(
    ...
    throw new Error(
      `Refresh not supported for playlist type: ${playlist.type}`,
    );
  }
    ...
    for (const playlist of searchPlaylists) {
      for (const channel of playlist!.channels) {
        if (
          ...
          results.push({...channel, playlistId: playlist!.id} as Channel & {
            playlistId: string;
          });
        }
      }
  ...
  private async savePlaylist(playlist: Playlist): Promise<void> {
    ...
    this.stats.totalChannels = Array.from(this.playlists.values()).reduce(
      (total, p) => total + p.totalChannels,
      0,
    );
  }
  ...
  private updateStats(parseTime: number): void {
    this.stats.averageParseTime =
      this.stats.averageParseTime * 0.9 + parseTime * 0.1;
   ...
   */
  private async getCachedPlaylistSafely(
    cacheKey: string,
    ...
    try {
      ...
      console.log(
        `🔍 Cache validation OK: ${playlist.channels.length} channels`,
      );
      return {playlist};
    } catch (error) {
   ...
   */
  private async cachePlaylistSafely(
    cacheKey: string,
    playlist: Playlist,
  ): Promise<void> {
    try {
  ...
  private validateCachedPlaylist(cached: any): boolean {
    try {
      ...
      const invalidChannels = playlist.channels.some(
        (channel: any, index: number) => {
          if (!channel || typeof channel !== 'object') {
            console.warn(
              `🔍 Cache validation: Channel ${index} is not an object`,
            );
            return true;
          }
          ...
          if (!channel.name || typeof channel.name !== 'string') {
            console.warn(
              `🔍 Cache validation: Channel ${index} missing valid name`,
            );
            return true;
          }
          ...
          if (!channel.url || typeof channel.url !== 'string') {
            console.warn(
              `🔍 Cache validation: Channel ${index} missing valid URL`,
            );
            return true;
          }
          ...
          if (!channel.id || typeof channel.id !== 'string') {
            console.warn(
              `🔍 Cache validation: Channel ${index} missing valid ID`,
            );
            return true;
          }
      ...
      console.log(
        `🔍 Cache validation: OK - ${playlist.channels.length} channels validated`,
      );
      return true;
    } catch (error) {
  ...
  async testEPGDetection(content: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        detectionMethod: result.metadata.epgUrl
          ? 'url-tvg detection'
          : undefined,
      };
    } catch (error) {
  ...
  async getBestEPGSource(playlistId: string): Promise<any> {
    ...
    return await this.epgManager.getBestEPGSource(
      playlistId,
      playlist.metadata,
    );
  }
