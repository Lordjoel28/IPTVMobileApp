...
class ContentRatingSystem {
  private static readonly RATINGS: {[key: string]: ContentRating} = {
    ...
    PG: {
      level: 'PG',
      age: 8,
      description: 'Accord parental souhaitable',
      countries: ['US', 'CA'],
    },
    'PG-13': {
      level: 'PG-13',
      age: 13,
      description: 'Déconseillé aux moins de 13 ans',
      countries: ['US'],
    },
    R: {
      level: 'R',
      age: 17,
      description: 'Interdit aux moins de 17 ans',
      countries: ['US'],
    },
    'NC-17': {
      level: 'NC-17',
      age: 18,
      description: 'Interdit aux moins de 18 ans',
      countries: ['US'],
    },
    ...
    PG: {
      level: 'PG',
      age: 8,
      description: 'Accord parental',
      countries: ['GB'],
    },
    '12': {
      level: '12',
      age: 12,
      description: 'Interdit aux moins de 12 ans',
      countries: ['GB', 'DE'],
    },
    '15': {
      level: '15',
      age: 15,
      description: 'Interdit aux moins de 15 ans',
      countries: ['GB'],
    },
    '18': {
      level: '18',
...
export class ParentalController {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.accessAttempts = attempts.filter(
        (attempt: AccessAttempt) => attempt.timestamp > sevenDaysAgo,
      );
    ...
    try {
      ...
      if (temporaryUnlock && this.isUnlockActive(temporaryUnlock)) {
        ...
        if (temporaryUnlock.unlockedCategories.includes(category)) {
          console.log(
            `🔓 Temporary unlock active for ${user.name}: ${category}`,
          );
          return {allowed: true};
        }
      }
      ...
      if (restrictions.maxContentRating) {
        ...
        if (category.toLowerCase().includes('adult')) {
          if (!ContentRatingSystem.isAgeAppropriate('R', 18)) {
            // Supposer âge adulte
  ...
  async requestTemporaryUnlock(
    ...
    try {
      ...
      console.log(
        `🔓 Temporary unlock granted for ${childUser.name}: ${categories.join(
          ', ',
        )} (${duration}min)`,
      );
  ...
  getActiveUnlocks(): TemporaryUnlock[] {
    return Array.from(this.temporaryUnlocks.values()).filter(unlock =>
      this.isUnlockActive(unlock),
    );
  }
  ...
  async updateSettings(
    newSettings: Partial<ParentalSettings>,
    currentPin?: string,
  ): Promise<boolean> {
    await this.initialize();
  ...
  private async logAccessAttempt(
    ...
    if (blocked) {
      ...
      const category = channel.category || 'Unknown';
      this.stats.categoriesBlocked[category] =
        (this.stats.categoriesBlocked[category] || 0) + 1;
      await this.saveStats();
    }
  }
  ...
  private async saveTemporaryUnlocks(): Promise<void> {
    const activeUnlocks = Array.from(this.temporaryUnlocks.values()).filter(
      unlock => this.isUnlockActive(unlock),
    );
  ...
  private updateStats(): void {
    ...
    let maxBlocks = 0;
    for (const [category, blocks] of Object.entries(
      this.stats.categoriesBlocked,
    )) {
      if (blocks > maxBlocks) {
        maxBlocks = blocks;
  ...
  async exportAccessLogs(): Promise<string> {
    ...
    const csv = [
      ...
      ...attempts.map(
        attempt =>
          `${new Date(attempt.timestamp).toISOString()},${attempt.userName},${
            attempt.channelName
          },${attempt.category},${attempt.blocked ? 'Oui' : 'Non'},"${
            attempt.reason
          }"`,
      ),
    ].join('\n');
