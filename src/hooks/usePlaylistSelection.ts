...
export const usePlaylistSelection = () => {
  ...
  const selectPlaylistWithAnimation = useCallback(async (
    playlistId: string,
    playlistName?: string
  ): Promise<Playlist | null> => {
    try {
      console.log('🎬 Début sélection playlist avec animation:', playlistName);
      
      // 🚀 ANIMATION IMMÉDIATE - Aucun délai !
      showLoading(
        playlistName ? `Chargement ${playlistName}...` : 'Chargement playlist...',
        'Lecture des chaînes...',
        0
      );
      ...
      console.error('❌ Hook: Erreur dans selectPlaylistWithAnimation:', error);
