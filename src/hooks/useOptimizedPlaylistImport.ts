...
export const useOptimizedPlaylistImport = () => {
   ...
   */
  const importPlaylistOptimized = useCallback(
    async (url: string, name: string): Promise<boolean> => {
      ...
      try {
          ...
          name,
          {
            ...
            onProgress: (progress: ParseProgress) => {
              ...
              setImportState(prev => ({
                ...
                currentDetails: `${Math.round(progress.parseSpeed)} ch/s • ${
        ...
        if (result.success) {
          ...
          showNotification('🎉 Import Ultra-Rapide Réussi !', 'success', 5000);

          showNotification(
            `📊 ${result.totalChannels} chaînes en ${Math.round(
              result.parseTimeMs / 1000,
            )}s (${result.avgSpeed} ch/s)`,
            'success',
            8000,
          );
          ...
          showNotification(`❌ Échec Import: ${errorMsg}`, 'error', 6000);
  ...
  const runPerformanceTest = useCallback(async () => {
    ...
    try {
      ...
      const summary = results
        .map(r =>
          r.success
            ? `✅ ${r.totalChannels} chaînes en ${Math.round(
                r.totalTimeMs / 1000,
              )}s (${r.avgSpeed} ch/s)`
            : `❌ Échec: ${r.error}`,
        )
      ...
      showNotification(`🧪 Benchmark Terminé:\n${summary}`, 'success', 10000);
